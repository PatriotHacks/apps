...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-6 rounded-xl border bg-card py-4 text-card-foreground shadow-sm sm:py-6",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex flex-wrap items-center gap-2 px-4 sm:px-6 [.border-t]:pt-6", className)}
      {...props}
    />
  )
}
